...
> [!NOTE]
> We have adopted "WebMeeting Software Suite" (https://meeting.dev/index.html) and (https://sora-cloud.shiguredo.jp/).
...
`create_access_token_type`<br>
アクセストークンの取得方法を指定します。
- `sora-cloud`（デフォルト）: Sora Cloudを使用します。
- `meeting.dev`: WebMeeting Software Suite (meeting.dev) を使用します。

`create_access_token_url`<br>
meeting.dev使用時のアクセストークン取得URLを指定します。

`push_channel_type`<br>
チャンネル通知の方法を指定します。
- `sora-cloud`（デフォルト）: Sora Cloudを使用します。
- `meeting.dev`: WebMeeting Software Suite (meeting.dev) を使用します。

`push_channel_url`<br>
meeting.dev使用時のチャンネル通知URLを指定します。
