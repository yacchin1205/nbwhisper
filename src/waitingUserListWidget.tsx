...
export class WaitingUserListWidget extends ReactWidget {
  ...
  _isVisibleOtherClientsTalking() {
    ...
    if (
      this._ownClient.state !== UserState.Talking &&
      this._ownClient.state !== UserState.Calling
    ) {
      const userState = this._ownUser.getState();
  ...
  render(): JSX.Element {
    return (
      <React.Fragment>
        <div
          className={`nbwhisper-waiting-user-list-widget ${
            this._isNotebook7 && 'notebook7'
          }`}
          hidden={this._isVisibleOtherClientsTalking()}
        >
          <Draggable>
            <div>
              ...
              {this._isListVisible && (
                <div
                  className={`nbwhisper-waiting-user-list-dialog ${
                    this._isNotebook7 && 'notebook7'
                  }`}
                >
                  <WaitingUserList
        ...
        {this._isVisibleOtherClientsTalking() && (
          <div
            className={`nbwhisper-waiting-user-list-hidden-text ${
              this._isNotebook7 && 'notebook7'
            }`}
          >
            他タブ・ウィンドウで通話中
